...
public class AndroidUtils {
    ...
    /**
     * Adds a click listener to the given view which invokes the method named by methodName on the
     * given target. The method must be public and take no arguments.
     */
    ...
    /**
     * Notifies the OS to index the new image, so it shows up in Gallery. Allows optional callback
     * method to notify client when the scan is completed, e.g. so it can access the "content" URI
     * that gets assigned.
     */
    public static void scanSavedMediaFile(final Context context, final String path,
            final MediaScannerCallback callback) {
        ...
        try {
            MediaScannerConnection.MediaScannerConnectionClient scannerClient =
    ...
    public static BitmapFactory.Options computeBitmapSizeFromURI(Context context, Uri imageURI)
        ...
        BitmapFactory.decodeStream(context.getContentResolver().openInputStream(imageURI), null,
                options);
        return options;
    }

    /**
     * Returns a Bitmap from the given URI that may be scaled by an integer factor to reduce its
     * size, while staying as least as large as the width and height parameters.
     */
    public static Bitmap scaledBitmapFromURIWithMinimumSize(Context context, Uri imageURI,
    ...
    /**
     * Given a width and height, fills output array with scaled width and height values such that
     * one of the values is exactly equal to the given maximum width or height, and the other value
    ...
    public static int[] scaledWidthAndHeightToMaximum(int width, int height, int maxWidth,
            int maxHeight) {
    ...
    /**
     * Returns the estimated memory usage in bytes for a bitmap. Calls bitmap.getByteCount() if that
     * method is available (in API level 12 or higher), otherwise returns 4 times the number of
