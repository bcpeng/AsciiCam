...
/**
 * This class implements a two-level cache for Bitmaps. The first level is an in-memory map which
 * uses an LruCache so that the Bitmaps will be freed when necessary. The second level is a location
 * directory on the SD card, where smaller versions of the images will be saved for faster
 * retrieval. The exact location is determined by the ThumbnailLocator implementation passed to the
 * constructor.
 */
    ...
    public Bitmap getScaledBitmap(Uri imageUri, int minWidth, int minHeight) {
        ...
        if (thumbfile != null && thumbfile.isFile()) {
            try {
                ...
                if (bitmap != null && bitmap.getWidth() >= minWidth &&
        ...
        try {
            bitmap = AndroidUtils.scaledBitmapFromURIWithMinimumSize(context, imageUri, minWidth,
                    minHeight);
        } catch (Exception ex) {
            bitmap = null;
        }
        if (bitmap != null) {
            ...
            try {
                ...
                (new File(thumbfile.getParentFile().getPath() + File.separator + ".nomedia"))
                        .createNewFile();
            } catch (Exception ignored) {
            }
        }
