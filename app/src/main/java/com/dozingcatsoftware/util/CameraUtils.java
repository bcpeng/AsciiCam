...
/**
 * This class contains useful methods for working with the camera in Android apps. The methods will
 * build and run under Android 1.6 or later; methods available only in later versions are called
 * using reflection.
 */
...
public class CameraUtils {
    ...
    /**
     * Returns the number of cameras accessible to the Android API. This is always 1 on platforms
     * earlier than Android 2.3, and on 2.3 or later it is the result of
    ...
    /**
     * Returns a list of available camera preview sizes, or null if the Android API to get the sizes
     * is not available.
     */
     ...
     */
    public static Camera.Size bestCameraSizeForWidthAndHeight(Camera.Parameters params, int width,
            int height) {
    ...
    /**
     * Returns a list of available camera picture sizes, or null if the Android API to get the sizes
     * is not available.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Sets the camera's picture size to the maximum available size, as determined by number of
     * pixels (width*height). Returns a Camera.Size object containing the updated picture size.
     */
    ...
    /**
     * Sets pixels in the given bitmap to a grayscale image from the byte array from a camera
     * preview, assumed to be in YUV (NV21) format with brightness pixels first.
     */
    public static Bitmap fillGrayscaleBitmapFromCameraData(Bitmap bitmap, byte[] cdata, int width,
            int height) {
    ...
    /**
     * Opens the camera with the given ID. If the Android API doesn't support multiple cameras (i.e.
     * prior to Android 2.3), always opens the primary camera.
     */
    ...
    /**
     * Attempts to allocate and register the given number of preview callback buffers. Uses the
     * camera's current preview size to determine the size of the buffers. If the Android API
     * doesn't support preview buffers, does nothing. Returns true if successful.
     */
    ...
    /**
     * Attempts to add the given byte array as a camera preview callback buffer. If the Android API
     * doesn't support preview buffers, does nothing and returns false. Returns true if successful.
     */
    ...
    /**
     * Sets the given callback object on the given camera. Calls setPreviewCallbackWithBuffer if the
     * Android API supports it, otherwise calls setPreviewCallback.
     */
    ...
    /**
     * Returns a list of available camera flash modes. If the Android API doesn't support getting
     * flash modes (requires 2.0 or later), returns a list with a single element of "off",
     * corresponding to Camera.Parameters.FLASH_MODE_OFF.
     */
    ...
    /**
     * Attempts to set the camera's flash mode. Returns true if successful, false if the Android API
     * doesn't support setting flash modes.
     */
    ...
    public static class CameraInfo {
        ...
        public boolean isRotated180Degrees() {
            ...
            return (isFrontFacing() && orientation == 90) ||
                    (!isFrontFacing() && orientation == 270);
        }
    }
    ...
    public static CameraInfo getCameraInfo(int cameraId) {
        try {
            ...
            Method getCameraId = Camera.class.getMethod("getCameraInfo", int.class,
                    cameraInfoClass);
